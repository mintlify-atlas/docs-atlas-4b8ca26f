...
The `generalizing x y` modifier reverts `x` and `y` before constructing the side goal, so the wlog-claim `this` can be applied to `x` and `y` in a different order (the typical symmetry use case).
