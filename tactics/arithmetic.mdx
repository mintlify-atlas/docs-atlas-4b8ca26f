...
```
  ...
  <Tab title="ring!">
```lean
-- ring! uses a stronger reducibility setting to identify atoms
-- It unfolds reducible definitions that plain ring would leave opaque

-- ring handles this directly:
example (a b c : ℝ) : (a + b) * c = a * c + b * c := by ring

-- ring! is needed when terms are definitionally equal but ring can't see it:
-- e.g., when a custom definition wraps a ring expression
def myDouble (n : ℤ) : ℤ := n + n
example (n : ℤ) : myDouble n = 2 * n := by ring!
```
  </Tab>
</Tabs>
