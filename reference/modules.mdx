...
description: "How Mathlib4 organizes its 8000+ modules across 33 top-level directories, how to use selective imports, and how to find theorems in the library."
---

Mathlib4 is structured as a collection of more than 8,000 individual Lean 4 modules, each corresponding to a single `.lean` source file. These modules are organized into 33 top-level directories under the `Mathlib/` folder, and every module is re-exported from the single root file `Mathlib.lean`. Understanding this hierarchy helps you import only what your project needs, keeps build times fast, and makes it easier to locate specific definitions and theorems in the library.
...
The 33 top-level directories in `Mathlib/` each group a coherent area of mathematics or infrastructure. The table below describes what each one contains.
...
<Accordion title="Interactive tactics inside a proof">
  ...
  -- Runs a general proof-search strategy
  search_proof_tactic
  ```
</Accordion>
...
<Accordion title="Loogle and Moogle semantic search">
  ...
  - Both are also available as Lean tactics via `LeanSearchClient`:

  ```lean
  import Mathlib.Tactic.Loogle
  #check @?   -- hover a declaration to reveal its full name
  ```
</Accordion>
